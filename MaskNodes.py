...
class MultiCutByMask:
    """
    Cuts the images to the bounding box of the mask. If force_resize_width or force_resize_height are provided,
    the image will be resized to those dimensions. The `mask_mapping_optional` input can be provided from a 
    'Separate Mask Components' node to cut multiple pieces out of a single image in a batch.
    """
    def __init__(self):
        pass
...
class MutilPasteByMask:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image_base": ("IMAGE",),
                "image_to_paste": ("IMAGE",),  # this will be a list of images from MultiCutByMask
                "mask": ("IMAGE",),
                "resize_behavior": (["resize", "keep_ratio_fill", "keep_ratio_fit", "source_size", "source_size_unmasked"],)
            },
            "optional": {
    ...
    def paste(self, image_base, image_to_paste, mask, resize_behavior, mask_mapping_optional=None):
        image_base = tensor2rgba(image_base)
        image_to_paste = [tensor2rgba(img) for img in image_to_paste]  # Convert list of images
        mask = tensor2mask(mask)

        # Scale the mask to be a matching size if it isn't
        B, H, W, C = image_base.shape
        MB = mask.shape[0]
        PB = len(image_to_paste)  # Number of images to paste
        ...
        if mask_mapping_optional is None:
            ...
            if PB < B:
                assert(B % PB == 0)
                image_to_paste = image_to_paste * (B // PB)  # Repeat the list to match batch size
        mask = torch.nn.functional.interpolate(mask.unsqueeze(1), size=(H, W), mode='nearest')[:, 0, :, :]
        MB, MH, MW = mask.shape
        ...
        for i in range(0, MB):
            if is_empty[i]:
                ...
                if mask_mapping_optional is not None:
                    image_index = mask_mapping_optional[i].item()
                source_size = image_to_paste[image_index].size()  # Get the corresponding image to paste
                SB, SH, SW, _ = image_to_paste[image_index].shape
                ...
                # Resize the image we're pasting if needed
                resized_image = image_to_paste[image_index].unsqueeze(0)
                if SH != height or SW != width:
                    resized_image = torch.nn.functional.interpolate(resized_image.permute(0, 3, 1, 2), size=(height, width), mode='bicubic').permute(0, 2, 3, 1)
...
NODE_CLASS_MAPPINGS = {
    ...
    "MultiCut By Mask": MultiCutByMask,
    "Paste By Mask": PasteByMask,
    "MultiPaste By Mask": MutilPasteByMask,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "MultiCut By Mask": "MultiCut By Mask",
    "Paste By Mask": "Paste By Mask",
    "MultiPaste By Mask": "MultiPaste By Mask",
